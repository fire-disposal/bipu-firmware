...
static const void* astra_font = NULL;
...
bool astra_bind_item_to_selector(astra_list_item_t *_item)
{
  ...
  // 兼容 main.c 用法：允许根节点直接绑定selector（即parent为NULL），但只在有子项时生效
  if (_item->parent == NULL) {
    if (_item->child_num > 0) {
      astra_selector.selected_index = 0;
      astra_selector.selected_item = _item->child_list_item[0];
      astra_selector.y_selector = 2 * SCREEN_HEIGHT;
      astra_selector.h_selector = 160;
      return true;
    } else {
      // 根节点无子项，无法初始化selector
      astra_selector.selected_index = 0;
  ...
  // 防御式：parent为NULL或child_num为0时直接返回
  if (_item->parent == NULL || _item->parent->child_num == 0)
    return false;
